# Evaluation contract — applypilot

Define success criteria per goal, at goal start (PROCESS.md step 1). Honest criteria are properties of the goal, not defaults baked into this file. This file holds how to phrase them, the standing verification commands for applypilot, and the rubric the reviewer scores against.
...
The session that installs the harness fills this list in for applypilot: build, test, lint, and end-to-end commands, each with its expected outcome. Every goal runs them in PROCESS.md step 4 and captures raw output into the journal.
